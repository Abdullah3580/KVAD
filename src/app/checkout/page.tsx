...
import { T, fmt, FREE_SHIP, DELIVERY_OPTS, PAY_METHODS, BD_DISTRICTS } from "@/lib/constants";
...
// Fix: server-side coupon validation via Supabase (no hardcoded COUPONS object exposed to client)
async function validateCoupon(code: string, subtotal: number): Promise<{ discount: number; msg: string }> {
  ...
  if (error || !data) return { discount: 0, msg: "অকার্যকর কোড ❌" };

  // Check expiry
  if (data.expires_at && new Date(data.expires_at) < new Date()) {
    return { discount: 0, msg: "কোডের মেয়াদ শেষ ❌" };
  }
  // Check min order
  if (data.min_order && subtotal < data.min_order) {
    return { discount: 0, msg: `ন্যূনতম অর্ডার ৳${data.min_order} হতে হবে` };
  }

  const pct = Number(data.discount ?? 0);
  const discountAmt = Math.round((subtotal * pct) / 100);
  return { discount: discountAmt, msg: `✓ ${pct}% ছাড় প্রয়োগ হয়েছে` };
}

// Fix: Better order number — timestamp + random to avoid collision
...
export default function CheckoutPage() {
  ...
  // Fix: get user for user_id in order
  const { user } = useAuth();
  ...
  // Fix: coupon validation goes to DB, not hardcoded client object
  const applyCode = async () => {
    if (!code.trim()) return;
    setCLoading(true);
    const result = await validateCoupon(code, sub);
    setDisc(result.discount);
    setCMsg(result.msg);
    setCLoading(false);
  };

  // Fix: phone validation regex
  const addrValid = addr.name && isValidPhone(addr.phone) && addr.line1 && addr.city;
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const { data: order, error } = await supabase.from("orders").insert({
        order_number: num,
        // Fix: user_id now saved
        user_id: user?.id ?? null,
        status: "pending",
        total: tot,
        subtotal: sub,
        discount: disc,
        shipping_cost: sub >= FREE_SHIP ? 0 : shipCost,
      ...
      const msg = e instanceof Error ? e.message : "অর্ডার হয়নি ❌";
      toast(msg, "err");
    } finally {
      setLoading(false);
    }
  ...
  if (done) return (
    <div style={{ maxWidth: 540, margin: "80px auto", padding: "0 20px", textAlign: "center" }}>
      ...
      <div style={{ background:T.raised, borderRadius:14, padding:"20px 28px", marginBottom:28, border:`1px solid ${T.border}` }}>
        ...
        <p className="playfair" style={{ fontSize:22, fontWeight:900, color:T.coral }}>{orderNum}</p>
      </div>
      <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
        <Btn onClick={() => router.push("/orders")} v="outline">অর্ডার দেখুন</Btn>
        <Btn onClick={() => router.push("/")}>আবার কেনাকাটা</Btn>
      </div>
    </div>
  );
  ...
  if (cart.length === 0) return (
    <div style={{ maxWidth:1400, margin:"0 auto", padding:"60px 20px" }}>
      ...
      <EmptyState icon="🛒" title="কার্ট খালি" sub="আগে পণ্য যোগ করুন।" action="শপে যান" onAction={() => router.push("/")} />
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:1200, margin:"0 auto", padding:"28px 20px 80px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr min(360px,100%)", gap:28 }}>
        <div>
          {/* ── STEP 1 ── */}
          {step===1 && (
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:16, padding:28 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                ...
                <div>
                  ...
                  {addr.phone && !isValidPhone(addr.phone) && (
                    <p style={{ fontSize:11, color:T.danger, marginTop:4 }}>সঠিক বাংলাদেশি নম্বর দিন (01XXXXXXXXX)</p>
                  )}
                </div>
            ...
            <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:16, padding:28 }}>
              ...
              <div style={{ background:T.raised, borderRadius:12, padding:16, marginBottom:20 }}>
                ...
                <div style={{ display:"flex", gap:8 }}>
                  <input value={code} onChange={e=>setCode(e.target.value)} onKeyDown={e=>e.key==="Enter"&&applyCode()} placeholder="কুপন কোড লিখুন"
